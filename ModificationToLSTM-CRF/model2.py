...
from utils2 import shared, set_values, get_name
from nn2 import HiddenLayer, EmbeddingLayer, DropoutLayer, LSTM, forward
...
class Model(object):
    ...
    def __init__(self, parameters=None, models_path=None, model_path=None):
        ...
        if model_path is None:
            ...
            # Model location
            model_path = os.path.join(models_path, self.name)[:40]
    ...
    def save_mappings(self, id_to_word, id_to_char, id_to_tag, id_to_postag, id_to_wordbigram,id_to_posbigram ,dico_tags):
        """
	...
	self.id_to_wordbigram = id_to_wordbigram
	self.id_to_posbigram = id_to_posbigram
	self.dico_tags = dico_tags
		...
		'id_to_wordbigram' : self.id_to_wordbigram,
		'id_to_posbigram' : self.id_to_posbigram,
	...
	self.id_to_wordbigram = mappings['id_to_wordbigram']
	self.id_to_posbigram = mappings['id_to_posbigram']
	self.dico_tags = mappings['dico_tags']
    ...
    def weighted_categorical_crossentropy(self, y_pred, y_true, weights):
     """
    ...
    def w_categorical_crossentropy(self, y_pred, y_true, weights):
	    nb_cl = len(weights)
		...
		#print c_p, c_t
		#print weights[c_p][c_t]
		#print y_pred_max_mat[:, c_p] * y_true[ c_t]
		final_mask += (weights[c_t][c_p] * y_pred_max_mat[:,c_p] * y_true[:])
	    #print final_mask
	    return T.nnet.categorical_crossentropy(y_pred, y_true) * final_mask
	...
	n_bigrams = len(self.id_to_wordbigram)
	n_posbigrams = len(self.id_to_posbigram)
	dico_tags = self.dico_tags
	#print dico_tags
	
	class_weights = [(1.0*dico_tags['O'])/(2*dico_tags[self.id_to_tag[i]]) for i in xrange(n_tags)]
	
	weights = np.ones((n_tags,n_tags))
	for c_p, c_t in product(range(n_tags), range(n_tags)):
		if(c_p == c_t ):
			weights[c_p][c_t] = -1 #max(class_weights[c_p], class_weights[c_t] )
	...
	wordbigram_ids = T.ivector(name='wordbigram_ids')
	posbigram_ids = T.ivector(name='posbigram_ids')
	clusters = T.ivector(name='clusters')
	...
	if postag_dim:
            #input_dim += postag_dim
            postag_layer = EmbeddingLayer(n_postags, postag_dim, name='postag_layer')
            final_output = T.concatenate([final_output, postag_layer.link(postag_ids)],axis=1 )

	if wordbigram_dim:
            #input_dim += postag_dim
            wordbigram_layer = EmbeddingLayer(n_bigrams, wordbigram_dim, name='wordbigram_layer')
            final_output = T.concatenate([final_output, wordbigram_layer.link(wordbigram_ids)], axis = 1 )

	if posbigram_dim:
            #input_dim += postag_dim
            posbigram_layer = EmbeddingLayer(n_bigrams, posbigram_dim, name='posbigram_layer')
            final_output = T.concatenate([final_output, posbigram_layer.link(posbigram_ids)], axis = 1 )

	if wordnet_cluster:
            #input_dim += postag_dim
            wordnet_layer = EmbeddingLayer(n_words, wordnet_cluster, name='wordnet_layer')
            final_output = T.concatenate([final_output, wordnet_layer.link(clusters)], axis = 1 )
        ...
        final_layer = HiddenLayer(word_lstm_dim + postag_dim + wordbigram_dim + posbigram_dim+wordnet_cluster, n_tags, name='final_layer',
                                  activation=(None if crf else 'softmax'))
        tags_scores = final_layer.link(final_output)
	
	#print tags_scores.ndim
	#print tag_ids.ndim
	    ...
	    #add_cost = 0.0
	    #for atid in tag_ids.eval():
	    #	add_cost += class_weights[atid]	    
	    #cost = self.w_categorical_crossentropy(tags_scores, tag_ids, weights).mean()
	...
	if postag_dim:
	    self.add_component(postag_layer)
            params.extend(postag_layer.params)
        if wordbigram_dim:
	    self.add_component(wordbigram_layer)
            params.extend(wordbigram_layer.params)
        if posbigram_dim:
	    self.add_component(posbigram_layer)
            params.extend(posbigram_layer.params)
	if wordnet_cluster:
	    self.add_component(wordnet_layer)
            params.extend(wordnet_layer.params)
	...
	if wordbigram_dim:
	    eval_inputs.append(wordbigram_ids)
        if posbigram_dim:
	    eval_inputs.append(posbigram_ids)
	if wordnet_cluster:
	    eval_inputs.append(clusters)
	...
	#theano.printing.pydotprint(cost, outfile="archect.png", var_with_name_simple=True)
